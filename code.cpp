...
static void doScrollCommand(){
    ...
    // First, perform an internal flush (without printing flush info), then output prompt and pre-scroll scoreboard
    performFlush(false);
    cout << "[Info]Scroll scoreboard.\n";
    // Output scoreboard before scrolling (this is after flushing)
    ...
    vector<int> rankIndexDummy; // not used
    ...
    // After scrolling ends, output scoreboard after scrolling
    // Frozen state is lifted
    G.frozen = false;
    for(auto &t: G.teams){
        for(int i=0;i<G.M;i++){
            auto &p = t.probs[i];
            if(p.fstate==ProblemState::Revealed) p.fstate = ProblemState::None;
            // Reset freeze markers for future cycles
            p.preSolvedAtFreeze = false;
            p.preFreezeWrong = 0;
            p.postFreezeSubmissions = 0;
        }
    ...
    // Recompute and output final scoreboard
    performFlush(false);
    printScoreboardByOrder(G.lastRanking);
}
